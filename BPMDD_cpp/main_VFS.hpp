...
void Problem::llenarComp(){ //map<int,vector<int>>Comp;//Successors  map<int,vector<int>>CompPred;//predecessor
    CompPred.clear();
    Comp.clear();
...
vector<pair<int,int>> Problem:: ordenDegre(){
    ...
    else if (DegreeType == "Best_K-VFS"){
        clock_t tStartCycleFinder = clock();
        MainCycleFinder();
        cout << "Time taken to find K-cycles: " << (clock() - tStartCycleFinder)/double(CLOCKS_PER_SEC) << endl;
        if (TooManyVFSCycles == false){
            vp = OptimalVFS();
        }
